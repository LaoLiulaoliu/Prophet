...
train_gt = dataset.get_training_data_gt_matrix(is_ranking=is_ranking, is_categorical=is_categorical, name="like")
print("shape: ", train_gt.shape)
val_gt = dataset.get_validation_data_gt_matrix(is_ranking=is_ranking, is_categorical=is_categorical, name="like")
...
for nb_row in ngrams:
  ...
  model.add_node(Convolution2D(nb_filter, nb_row, nb_col, init="normal", subsample=(1,nb_col), border_mode="valid",
...
model.add_node(Dropout(0.5), name='conv_drop1', inputs=conv_output_names, merge_mode='concat', concat_axis=1)
...
model.add_node(Dropout(0.6), name='conv_drop2', input='conv2_dense1')
model.add_node(Dense(4096, init="uniform", activation="relu",
                     W_regularizer=l2(0.01), W_constraint = maxnorm(max_norm_v)),
...
model.add_node(Dropout(0.6), name='mix_drop1', inputs=['ppl_flatten','conv2_dense2'], merge_mode='concat', concat_axis=1)
model.add_node(Dense(4096, init="uniform", activation="relu",
                     W_regularizer=l2(0.01), W_constraint = maxnorm(max_norm_v)),
               name="mix_dense1", input='mix_drop1'
               )
model.add_node(Dropout(0.6), name='mix_drop2', input='mix_dense1')

# now, let's do softmax
max_ranks_len = dataset.max_ranks()
model.add_node(Dense(max_ranks_len, init="normal", activation='softmax',
                  W_regularizer=l2(0.01), W_constraint = maxnorm(max_norm_v)), name='softmax3_pre', input='mix_drop2')
#model.add_node(Dense(max_ranks_len, init="normal", activation='softmax',
...
checkpoint = ModelCheckpoint(save_dir+"/conv2d_softmax_forward.full_t.pkl", save_best_only=False)
if is_ranking:
  precision = WeiboPrecisionCallback(1, 1, dataset_ranking=dataset, val_saved_filename="./test_saved_ppl_softmax.val", is_acc=True)
else:
  precision = WeiboPrecisionCallback(1, 1, is_acc=True)

model.fit({'words_input':train_words, 'ppl_input':train_ppl, 'softmax3':train_gt}, 
          batch_size=64, nb_epoch=121, callbacks=[checkpoint, precision], 
